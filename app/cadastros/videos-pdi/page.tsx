...
export default function VideosPdi() {
  ...
  const carregarVideos = async () => {
    try {
      ...
      console.error('Erro ao carregar os vídeos do PDI:', error);
    }
  };
  ...
  const saveData = async (id: number | null, formData: any) => {
    ...
    try {
      if (id) {
        ...
        if (response) {
          toast.success("Vídeo do PDI atualizado com sucesso.");
        ...
        if (response) {
          toast.success("Vídeo do PDI criado com sucesso.");
          await carregarVideos();
          return { success: true };
        }
      }
  ...
  const deleteData = async (id: number) => {
    try {
      await destroy(id);
      toast.success("Vídeo do PDI excluído com sucesso.");
      await carregarVideos();
      return { success: true };
    } catch (error) {
  ...
  const columns = [
    ...
    { dataField: 'titulo', label: 'Título' },
    {
      ...
      label: 'Competência',
      render: (_: string, item: VideoPdiData) => item.competencia?.descricao || 'Não informado',
    },
    {
      dataField: 'link',
      label: 'Link',
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Gerencie os vídeos recomendados para o plano de desenvolvimento individual.
        </CardDescription>
      </CardHeader>
